...
<template>
  <article class="prose-column">
    ...
    <p class="mt-3 text-muted">
      Short observations, derivations, and small experiments that do not need
      a full interactive piece.
    </p>

    <ul v-if="notes && notes.length" class="mt-10 space-y-6 rule-top pt-8">
      <li v-for="(n, i) in notes" :key="n.path" class="grid grid-cols-1 sm:grid-cols-[8rem_minmax(0,1fr)] gap-1 sm:gap-4 items-baseline">
